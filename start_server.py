...
from models import MatchSettings, AuthoritativeMatch, archive_object
...
def create_match():
    ...
    am = AuthoritativeMatch.from_match_settings(settings)

    obj_id, archived_am = archive_object(am, "AuthoritativeMatch", "match_1")

    payload = {
        "data": {obj_id: archived_am},
        "$root": {"$ref": obj_id}
    }
    return jsonify(payload)
